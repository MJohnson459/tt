...
pub fn report(
    store: &Store,
    month_str: Option<&str>,
    client: Option<String>,
    output: Option<&std::path::Path>,
) -> anyhow::Result<()> {
    // Parse or default to current month.
    let (year, month_num) = match month_str {
        Some(s) => {
            let (y, m) = s.split_once('-')
                .ok_or_else(|| anyhow!("invalid month '{}' — use YYYY-MM", s))?;
            let y = y.parse::<i32>().map_err(|_| anyhow!("invalid year in '{}'", s))?;
            let m = m.parse::<u32>().map_err(|_| anyhow!("invalid month in '{}'", s))?;
            if !(1..=12).contains(&m) {
                bail!("month must be 01–12");
            }
            (y, m)
        }
        None => {
    ...
    let period_start = NaiveDate::from_ymd_opt(year, month_num, 1)
        .unwrap()
        .and_hms_opt(0, 0, 0)
        .unwrap()
    ...
    let sessions: Vec<&Session> = store
        ...
        .iter()
        .filter(|s| {
            s.start >= period_start
                && s.start < period_end
                && s.end.is_some()
                && client.as_deref().map_or(true, |c| s.client == c)
        })
        .collect();

    if sessions.is_empty() {
        bail!("no completed sessions for {}", period_label);
    }

    // Collect clients in alphabetical order.
    let mut client_names: Vec<String> = {
        let mut seen = std::collections::HashSet::new();
        sessions.iter().map(|s| s.client.clone()).filter(|c| seen.insert(c.clone())).collect()
    };
    client_names.sort();
    ...
    for client_name in &client_names {
        let client_sessions: Vec<&&Session> = sessions.iter().filter(|s| s.client == *client_name).collect();
        let info = store.clients.get(client_name);
        let currency = info.map(|i| i.currency.as_str()).unwrap_or("USD");
        let rate = info.map(|i| i.rate).unwrap_or(0.0);

        let total_hours: f64 = client_sessions.iter().map(|s| s.duration_hours()).sum();
        let total_earnings: f64 = client_sessions.iter().map(|s| s.earnings()).sum();
